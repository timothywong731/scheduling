...
Efficiently managing travel and job scheduling for multiple resources across various locations presents a significant operational challenge. This tutorial showcases a Linear Programming (LP) model designed to optimise route planning and job allocation among travelling agents, aiming to minimise travel time and adhere to individual working hours constraints. Utilising variables such as travel costs, job durations, and agent capacities, we construct a mathematical framework that accommodates each agent's starting location and individual contractual obligations. The methodology employs the `MILPModel` in R, leveraging the `symphony` solver to identify optimal scheduling solutions. Experimental results, visualised through Gantt charts and geographical plotting, demonstrate the model's efficacy in reducing total travel time while ensuring feasible workload distribution. This approach not only enhances operational efficiency but also contributes to the broader field of operations research by providing a scalable solution for multi-location, multi-personnel scheduling problems.
